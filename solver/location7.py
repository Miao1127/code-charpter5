...
def confidence_matrix(u_position, c_t_n, t_sigma, s_path, beta_l=0.3, beta_m=0.5, beta_u=0.8, s_threshold_rate=0.1):
    """
    求解支持度矩阵
...
def repeatability_value_2(a_position, u1_position, u2_position, u3_position, u4_position, u1_sigma, u2_sigma,
    ...
    a = math.log(2 * np.pi * np.e * k_12_x)
    b = math.log(2 * np.pi * np.e * k_11_x)
    c = math.log(2 * np.pi * np.e * k_22_x)
...
def main():
    ...
    for i in range(5, usv_num+1):
        ...
        support_m_final, list_combination, position_est_b, position_est_noise \
            = confidence_matrix(usv_position, time_c_n, time_sigma, save_path, s_threshold_rate=0.01)
        ...
        # 去除重复度的最佳融合估计
        [auv_x_position_2, auv_y_position_2] = auv_position_est_2(support_m_final, position_est_b, list_combination,
                                                              auv_position, usv_position, time_sigma)
