        ...
        for j in range(combine_num):
            ...
            if j > i:  # 上三角区域
                ...
                # 重复输入数据为2个
                    ...
                    confidence_m_x[i][j] = np.sqrt((position_est[i][0] - position_est[j][0]) ** 2
                                                   + (position_est[i][1] - position_est[j][0]) ** 2) / ((1 - 0.5*b_x_2) * (
                            sig2_i + sig2_j))
                    confidence_m_y[i][j] = np.sqrt((position_est[i][0] - position_est[j][0]) ** 2
                                                   + (position_est[i][1] - position_est[j][0]) ** 2) / ((1 - 0.5*b_y_2) * (
                            sig2_i + sig2_j))
...
def auv_position_est_2(theta_x_m, theta_y_m, p_est_x, p_est_y):
    ...
    auv_y = w_y @ np.array(position_est_y[:, 0])  # y轴上auv位置融合估计结果
    return [auv_x, auv_y]
...
def main():
    ...
    for i in range(3, usv_num+1):
        ...
        support_m_final, list_combination, position_est_b, position_est_noise \
            = confidence_matrix_1(usv_position, time_c_n, time_sigma, save_path, s_threshold_rate=0.02)
        ...
        support_m_final_x, list_combination_x, position_est_b_x, support_m_final_y, list_combination_y, position_est_b_y\
            , position_est_noise = confidence_matrix_2(auv_position, usv_position, time_c_n, time_sigma, save_path, s_threshold_rate=0.2)
