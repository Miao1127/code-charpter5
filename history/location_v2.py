    ...
    for i in range(u_num):
        if d[i] > 9:
            ...
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                ...
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
    list2json.run(c_t, str(u_num)+'time_noise.json', s_path)
    list2json.run(t_sigma, str(u_num)+'t_sigma.json', s_path)
    return usv_position, c_t, c_t_n, t_sigma


def confidence_matrix(u_position, c_t_n, t_sigma, s_path, beta_l=0.3, beta_m=0.5, beta_u=0.8, s_threshold_rate=0.09):
    """
    求解支持度矩阵
...
def auv_position_est_2(s_m_f, p_est, c_list, a_position, u_position, t_sigma):
    """
    基于支持度的位置融合估计，去除重复度
    :param s_m_f: 统计支持度矩阵
    :param p_est: 最优位置融合估计列表
    :param c_list: 最佳融合估计列表
    :param a_position: 基于运动模型的auv位置估计
    :param u_position: usv位置
    :param t_sigma: 时差噪声
    :return:auv位置坐标
    """
    theta_x_m = deepcopy(s_m_f)
    theta_y_m = deepcopy(s_m_f)
    # 遍历最佳融合估计列表，将存在重复的情况取出
    for i in range(len(c_list)):
        if i + 1 < len(c_list):
            for j in range(i+1, len(c_list)):
                same = [x for x in c_list[i] if x in c_list[j]]
                dif1 = [x for x in c_list[i] if x not in c_list[j]]
                dif2 = [x for x in c_list[j] if x not in c_list[i]]
                # 重复输入数据为1个
                if len(same) == 1:
                    ...
                    t4 = t_sigma[dif2[0]]
                    t5 = t_sigma[dif2[1]]
                    [b_x_1, b_y_1] = repeatability_value_1(a_position, u1, u2, u3, u4, u5, t1, t2, t3, t4, t5)
                    theta_x_m[i][j] = (1-0.5*b_x_1)*theta_x_m[i][j]
                    theta_x_m[j][i] = theta_x_m[i][j]
                    theta_y_m[i][j] = (1 - 0.5 * b_y_1) * theta_y_m[i][j]
                    theta_y_m[j][i] = theta_y_m[i][j]
                # 重复输入数据为2个
                elif len(same) == 2:
                    u1 = u_position[dif1[0]]
                    ...
                    [b_x_1, b_y_1] = repeatability_value_2(a_position, u1, u2, u3, u4, t1, t2, t3, t4)
                    theta_x_m[i][j] = (1 - b_x_1) * theta_x_m[i][j]
                    theta_x_m[j][i] = theta_x_m[i][j]
                    theta_y_m[i][j] = (1 - b_y_1) * theta_y_m[i][j]
                    theta_y_m[j][i] = theta_y_m[i][j]
    ...
    position_est = np.array(p_est)
    auv_x = w_x @ np.array(position_est[:, 0])  # x轴上auv位置融合估计结果
    auv_y = w_y @ np.array(position_est[:, 1])  # y轴上auv位置融合估计结果
    return [auv_x, auv_y]
...
def main():
    ...
    for i in range(5, usv_num+1):
        ...
        support_m_final, list_combination, position_est_b, position_est_noise \
            = confidence_matrix(usv_position, time_c_n, time_sigma, save_path, s_threshold_rate=0.02)
        ...
        # 支持度矩阵计算
        support_m_final, list_combination, position_est_b, position_est_noise \
            = confidence_matrix(usv_position, time_c_n, time_sigma, save_path, s_threshold_rate=0.2)

        # 去除重复度的最佳融合估计
        [auv_x_position_2, auv_y_position_2] = auv_position_est_1(support_m_final, position_est_b)
        [sigma_x_2, sigma_y_2] = sigma_position(position_est_b, [auv_x_position_1, auv_y_position_1])
        print("去掉重复度的位置估计：")
