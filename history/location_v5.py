...
def main():
    ...
    # 读取已有的usv数据
    u_data = read_json.run('E:/博士论文试验数据/chapter5/论文采用数据/50usv_position.json')
    t_data = read_json.run('E:/博士论文试验数据/chapter5/论文采用数据/50time_noise.json')
    t_sigma_data = read_json.run('E:/博士论文试验数据/chapter5/论文采用数据/50t_sigma.json')
    ...
    while new_usv_num > usv_min:
        ...
        # 去掉支持度最低的usv
        for kk in range(int(usv_num*0.1)):
            count_min = np.inf
            count_index_s = 0
