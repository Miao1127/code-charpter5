...
# 开发人员：103中山分队-苗润龙
# 开发时间：20/11/3021:53
# 文件名：iteration_time_distance.py
# 开发工具：PyCharm
# 功能：缩小auv定位范围
...
def generate_data(u_num, a_position, s_path):
    ...
    for i in range(u_num):
        if d[i] > 9:
            t_sigma[i] = np.random.uniform(0.9, 1)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.9, 1)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 9 > d[i] >= 8:
            t_sigma[i] = np.random.uniform(0.8, 0.9)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.8, 0.9)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 8 > d[i] >= 7:
            t_sigma[i] = np.random.uniform(0.7, 0.8)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.7, 0.8)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 7 > d[i] >= 6:
            t_sigma[i] = np.random.uniform(0.6, 0.7)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.6, 0.7)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 6 > d[i] >= 5:
            t_sigma[i] = np.random.uniform(0.5, 0.6)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.5, 0.6)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 5 > d[i] >= 4:
            t_sigma[i] = np.random.uniform(0.4, 0.5)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.4, 0.5)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 4 > d[i] >= 3:
            t_sigma[i] = np.random.uniform(0.3, 0.4)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.3, 0.4)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 3 > d[i] >= 2:
            t_sigma[i] = np.random.uniform(0.2, 0.3)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.2, 0.3)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 2 > d[i] >= 1:
            t_sigma[i] = np.random.uniform(0.1, 0.2)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.1, 0.2)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
        elif 1 > d[i] >= 0:
            t_sigma[i] = np.random.uniform(0.01, 0.1)           # 随机生成噪声方差
            c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)   # 以一定方差生成时差噪声
            while c_t_n[i] < c_t[i]:  # 由于水声通信存在延时，带噪声的测量值一定大于真实值
                t_sigma[i] = np.random.uniform(0.01, 0.1)
                c_t_n[i] = c_t[i] + t_sigma[i] * np.random.randn(1)
...
def main():
    ...
    save_file_name = save_path + 'result_time_distance.csv'
    usv_position_save = save_path + 'usv_position_interval.csv'
    # 生成新的usv数据
    # u_data, time_c, t_data, t_sigma_data = generate_data(usv_num, auv_position, save_path)
    # 读取已有的usv数据
    u_data = read_json.run('E:/博士论文试验数据/chapter4/1606483803/50usv_position.json')
    t_data = read_json.run('E:/博士论文试验数据/chapter4/1606483803/50time_noise.json')
    t_sigma_data = read_json.run('E:/博士论文试验数据/chapter4/1606483803/50t_sigma.json')
    # 在上述求解的基础上，确定合适的usv数目，逐渐缩小定位区域范围，实现usv数目的逐渐减少
    # 加载最大数目usv的数据
    usv_position = u_data[:usv_num]
    time_c_n = t_data[:usv_num]
    time_sigma = t_sigma_data[:usv_num]
    # 未迭代范围时，最远usv的距离和时差
    t_interval = 0
    usv_furthest = 0
    for num in range(usv_num):
        distance = np.sqrt(
            (usv_position[num][0] - auv_position[0]) ** 2 + (usv_position[num][1] - auv_position[1]) ** 2 +
            auv_position[2] ** 2)
        if distance > usv_furthest:
            usv_furthest = distance
            t_interval = time_c_n[num]
    data = [0, 0, 0, 0, usv_num, t_interval, usv_furthest]
    print(data)
    ...
    position_data = []
    for num in range(usv_num):
        position_data.append(usv_position[num][0])
        position_data.append(usv_position[num][1])
        data2csv = pd.DataFrame([position_data])
        data2csv.to_csv(usv_position_save, mode='a', header=False, index=None)
        position_data = []
    ...
    usv_min = 5   # 最少usv数目
    support_threshold_rate = 0.1
    new_usv_num = usv_num
    while new_usv_num > usv_min:
        # 1.确定usv编号，在组合列表中找到距离最远的usv，并求出最远usv测得时差信息
        # 支持度矩阵计算
        support_m_final_x, list_combination_x, position_est_b_x, support_m_final_y, list_combination_y, position_est_b_y \
            , position_est_noise = confidence_matrix_2(auv_position, usv_position, time_c_n, time_sigma, save_path,
                                                       s_threshold_rate=support_threshold_rate)

        # 去除重复度的最佳融合估计
        [auv_x_position_2, auv_y_position_2] = auv_position_est_2(support_m_final_x, support_m_final_y, position_est_b_x,
                                                                  position_est_b_y)
        [sigma_x_2, sigma_y_2] = sigma_position(position_est_b_x, [auv_x_position_2, auv_y_position_2])

        # usv最佳列表元素
        list_combination = []   # 用于存储参与去掉重复度的最佳融合位置的usv编号组合
        for i in range(len(list_combination_x)):
            list_combination_x[i] = list(list_combination_x[i])
            list_combination = list_combination + list_combination_x[i]
        list_usv_num = list(set(list_combination))   # usv编号列表
        # 去掉支持度最低的usv
        for kk in range(int(new_usv_num*0.1)):
            count_min = np.inf
            count_index_s = 0
            for k in list_usv_num:
                if count_min > list_combination.count(k):
                    count_min = list_combination.count(k)
                    count_index_s = k
            list_usv_num.remove(count_index_s)
        # 2.将上述距离范围内的usv定位距离作为下次迭代的输入，采用相同的阈值参数
        # 求参与定位的最远usv与auv的距离
        usv_furthest = 0
        for num in list_usv_num:   # 确定最远usv
            new_usv_furthest = np.sqrt(
                (usv_position[num][0] - auv_position[0]) ** 2 + (usv_position[num][1] - auv_position[1]) ** 2 +
                auv_position[2] ** 2)
            if new_usv_furthest > usv_furthest:
                usv_furthest = new_usv_furthest
        # 将距离最远usv附近的usv编号过滤掉
        for num in list_usv_num:
            distance = np.sqrt(
                (usv_position[num][0] - auv_position[0]) ** 2 + (usv_position[num][1] - auv_position[1]) ** 2 +
                auv_position[2] ** 2)
            if distance >= usv_furthest:
                list_usv_num.remove(num)      # 去掉最远距离外的usv
        # 重新计算最远的usv距离和该usv测得的时差信息
        t_interval = 0
        usv_furthest = 0
        for num in list_usv_num:
            distance = np.sqrt(
            ...
            if distance > usv_furthest:
                usv_furthest = distance
                t_interval = time_c_n[num]
        # 3.判断usv的数目，是否到达最小的定位数目
        new_usv_num = len(list_usv_num)
        print(list_usv_num)
        print(new_usv_num)
        position_data = []
        for num in list_usv_num:
        ...
        time_sigma = []
        for num in list_usv_num:
            usv_position.append(u_data[num])
            time_c_n.append(t_data[num])
            time_sigma.append(t_sigma_data[num])

        # 保存数据
        data = [auv_x_position_2, auv_y_position_2, sigma_x_2, sigma_y_2, new_usv_num, t_interval, usv_furthest]
        print(data)
        data2csv = pd.DataFrame([data])
        data2csv.to_csv(save_file_name, mode='a', header=False, index=None)
